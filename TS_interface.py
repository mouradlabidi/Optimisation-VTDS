...
import time
from TS import tabu_search
from TS import plot_objective_function_over_time
from TS import write_execution_time
...
class TS_Meta(customtkinter.CTk):
    def __init__(self):
        super().__init__()
        ...
        self.initial_tabu_tenure = None
        self.nombre_de_solutions_voisines = None
        self.tempsExcute = None 
        self.poidEnergy = None 
        self.poidDelay = None

        # configure window
        self.title("La Recherche Tabou")
        ...
        #title
        self.Metaheuristic=customtkinter.CTkLabel(self, text="Métaheuristique: Recherche Tabou", font=customtkinter.CTkFont(size=20, weight="bold"))
        ...
        second_tab="Les paramètres de la recherche tabou "
        ...
        self.label_TabuTenure = customtkinter.CTkLabel(self.tabview.tab(second_tab), text="Durée tabou initiale", font=customtkinter.CTkFont(size=13, weight="bold"))
        self.label_TabuTenure.grid(row=0, column=0, padx=20, pady=20)
        self.TabuTenure = customtkinter.CTkEntry(self.tabview.tab(second_tab), placeholder_text="Durée tabou")
        self.TabuTenure.grid(row=0, column=1, padx=20, pady=20)

        self.label_NeighborSolutions = customtkinter.CTkLabel(self.tabview.tab(second_tab), text="Nombre de solutions voisines", font=customtkinter.CTkFont(size=13, weight="bold"))
        self.label_NeighborSolutions.grid(row=1, column=0, padx=20, pady=20)
        self.NeighborSolutions = customtkinter.CTkEntry(self.tabview.tab(second_tab), placeholder_text="Solutions voisines")
        self.NeighborSolutions.grid(row=1, column=1, padx=20, pady=20)
    ...
    def getParameters(self):
        try:
            ...
            self.initial_tabu_tenure = int(self.TabuTenure.get())
            self.nombre_de_solutions_voisines = int(self.NeighborSolutions.get())
        ...
        ###############################################
        if self.initial_tabu_tenure > 20 or self.initial_tabu_tenure < 5:
            messagebox.showerror("Erreur", "La durée tabou initiale doit être comprise entre 5 et 20")
            return False

        if self.nombre_de_solutions_voisines < 3 or self.nombre_de_solutions_voisines > 50:
            messagebox.showerror("Erreur", "Le nombre de solutions voisines doit être compris entre 3 et 50")
            return False
    ...
    def optimisationEvent(self):
        ...
        def run_TS_wrapper():
            self.run_TS(prog_window, start_time)
            # Une fois que run_TS est terminé, exécuter callback dans le thread principal
            prog_window.after(0, lambda: self.callback(start_time, prog_window))

        # Créer un Timer pour exécuter run_TS après un court délai
        ts_timer = threading.Timer(0.1, run_TS_wrapper)
        ts_timer.start()
    ...
    def callback(self, start_time, prog_window):
        ...
        self.show_results(self.meilleure_solution, self.best_objective_value, self.EC, self.Wbar, self.file_pathBest, self.file_pathCurrent, result_directory, filename)
            
    def run_TS(self, prog_window, start_time):
        ...
        nbVoisine = self.nombre_de_solutions_voisines
        self.file_pathBest = "best.xlsx"
        self.file_pathCurrent = "current.xlsx"
        execution_time = self.tempsExcute
        meilleure_valeur_objective = float('inf') 
        
        try:
            
            print("start TS")
            self.meilleure_solution, self.best_objective_value, self.EC, self.Wbar = tabu_search(self.espaces_recherche, ECmax, WbarMax, self.poid, max_iterations, execution_time, self.file_pathBest, self.file_pathCurrent, self.initial_tabu_tenure, nbVoisine, start_time)
            print("end TS")
            print("Meilleure solution trouvée :", self.meilleure_solution)
            print("Valeur de la fonction objectif :", self.best_objective_value) 
             
        except KeyboardInterrupt:
            print("Execution interrupted.")
            
            if self.meilleure_solution is not None:
                print("Meilleure solution trouvée :", self.meilleure_solution)
                print("Valeur de la fonction objectif :", self.best_objective_value) 
                

    def show_results(self, meilleure_solution, best_objective_value, EC, Wbar, file_pathBest, file_pathCurrent, result_directory, filename):
        import OptimisationResults
        import os
        ...
        if not destination_directory:
            ...
            def create_unique_subdirectory(directory):
                base_dir = os.path.join(directory, "resultTS")
                counter = 1
                new_dir = base_dir
        ...
        result_window = OptimisationResults.OptimisationResult(meilleure_solution, best_objective_value, EC, Wbar, self.image, result_directory)
